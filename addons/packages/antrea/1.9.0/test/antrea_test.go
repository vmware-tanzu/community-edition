...
const ipsecMode = "ipsec"

var (
	configName    = "antrea-config"
	configTweaker = "antrea-agent-tweaker"
...
var _ = Describe("Antrea YTT Templates", func() {
	...
	Context("antrea components with default configuration", func() {
		It("renders multiple configMap with a default IPAM configuration", func() {
...
`))
	...
	Describe("Configuring Multicast", func() {
		Context("without feature gate", func() {
			BeforeEach(func() {
				values, err = marshalAntreaConfig(fileValuesYaml, func(config *AntreaConfig) {
					config.Antrea.Config.FeatureGates.Multicast = false
				})
				Expect(err).NotTo(HaveOccurred())
			})
			...
			It("ignores the configuration", func() {
				...
				configMap := findConfigMapByName(unmarshalConfigMaps(output), configName)
				Expect(configMap).NotTo(BeNil())
				Expect(configMap.Data["antrea-agent.conf"]).To(ContainSubstring("Multicast: false"))
				Expect(configMap.Data["antrea-agent.conf"]).ToNot(ContainSubstring("igmpQueryInterval"))
				Expect(configMap.Data["antrea-agent.conf"]).ToNot(ContainSubstring("multicastInterfaces"))
			})
		})
		...
		Context("with the feature gate enabled", func() {
			...
			BeforeEach(func() {
				values, err = marshalAntreaConfig(fileValuesYaml, func(config *AntreaConfig) {
					config.Antrea.Config.FeatureGates.Multicast = true
					config.Antrea.Config.Multicast.IGMPQueryInterval = interval
					config.Antrea.Config.MulticastInterfaces = interfaces
				})
				Expect(err).NotTo(HaveOccurred())
			})
			...
			It("settings are configured", func() {
				...
				for _, s := range []string{"Multicast: true", interval, interfaces[0]} {
					Expect(configMap.Data["antrea-agent.conf"]).To(ContainSubstring(s))
				}
			})
		})
	...
	Describe("Configuring Multicluster", func() {
		Context("with the feature gate enabled", func() {
			ns := "kube-system"
			BeforeEach(func() {
				values, err = marshalAntreaConfig(fileValuesYaml, func(config *AntreaConfig) {
					config.Antrea.Config.FeatureGates.Multicluster = true
					config.Antrea.Config.MultiCluster.Enable = true
					config.Antrea.Config.MultiCluster.Namespace = ns
				})
				Expect(err).NotTo(HaveOccurred())
			})
		...
		Context("should be allowed", func() {
			BeforeEach(func() {
				values, err = marshalAntreaConfig(fileValuesYaml, func(config *AntreaConfig) {
					...
					config.Antrea.Config.FeatureGates.Multicast = true
					config.Antrea.Config.DNSServerOverride = "localhost"
					config.Antrea.Config.EnableBridgingMode = true
					config.Antrea.Config.DisableTXChecksumOffload = true
				})
				Expect(err).NotTo(HaveOccurred())
			})
