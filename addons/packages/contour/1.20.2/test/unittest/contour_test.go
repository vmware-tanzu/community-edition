...
var _ = Describe("Contour Ytt Templates", func() {
	var (
		...
		configDir = filepath.Join(repo.RootDir(), "addons/packages/contour/1.20.2/bundle/config")
	)
	...
	It("sets contour serve flags to ensure ipv6 listen addresses", func() {
		Expect(err).NotTo(HaveOccurred())
	...
	Context("Namespace set", func() {
		...
		It("changes all resources' namespaces", func() {
			...
			// Note, the overlay superficially indicates 13 matches (11 + 2), but we only expect 12
			// documents because the RoleBinding has two instances of the namespace, one for the resource
			// itself and one for the subject reference.
			docs = findDocsContainingLines(output, "\n  namespace: non-default-namespace")
			Expect(docs).To(HaveLen(12))
