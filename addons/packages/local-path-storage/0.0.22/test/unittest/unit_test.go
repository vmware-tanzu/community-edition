...
var _ = Describe("local-path-storage Ytt Templates", func() {
	var (
		values string
		output string
		err    error
	...
	JustBeforeEach(func() {
		...
		filePaths = append(filePaths,
			filepath.Join(repo.RootDir(), "addons/packages/local-path-storage/0.0.22/test/unittest/fixtures/values/default.yaml"),
		)
