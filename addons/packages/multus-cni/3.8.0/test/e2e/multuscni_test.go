...
var _ = Describe("Multus CNI Addon E2E Test", func() {
	BeforeEach(func() {
		...
		_, err = utils.Kubectl(nil, "get", "daemonset", "kube-multus-ds", "-n", "kube-system", "-o", "jsonpath='{range .spec.template.spec.containers[*]}{.image}{\"\\n\"}{end}'")
		Expect(err).NotTo(HaveOccurred())
	})
	...
	AfterEach(func() {
		...
		utils.ValidateDaemonsetNotFound("kube-system", "kube-multus-ds")
