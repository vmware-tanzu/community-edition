...
var _ = Describe("Harbor Ytt Templates", func() {
	var (
		...
		configDir = filepath.Join(repo.RootDir(), "addons/packages/harbor/2.6.2/bundle/config")

		ValuesFromFile = func(filename string) string {
			data, err := os.ReadFile(filepath.Join(repo.RootDir(), "addons/packages/harbor/2.6.2/test/unittest/fixtures/values", filename))
		...
		ExpectOutputEqualToFile = func(filename string) {
			data, err := os.ReadFile(filepath.Join(repo.RootDir(), "addons/packages/harbor/2.6.2/test/unittest/fixtures/expected", filename))
			Expect(err).NotTo(HaveOccurred())
	...
	JustBeforeEach(func() {
		...
		filePaths = append(filePaths,
			filepath.Join(repo.RootDir(), "addons/packages/harbor/2.6.2/test/unittest/fixtures/values/default.yaml"),
		)
