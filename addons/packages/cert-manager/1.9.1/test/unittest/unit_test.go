...
var _ = Describe("cert-manager Ytt Templates", func() {
	var (
		values string
		output string
		err    error

		configDir = filepath.Join(repo.RootDir(), "addons/packages/cert-manager/1.9.1/bundle/config")

		ValuesFromFile = func(filename string) string {
			data, err := os.ReadFile(filepath.Join(repo.RootDir(), "addons/packages/cert-manager/1.9.1/test/unittest/fixtures/values", filename))
		...
		ExpectOutputEqualToFile = func(filename string) {
			data, err := os.ReadFile(filepath.Join(repo.RootDir(), "addons/packages/cert-manager/1.9.1/test/unittest/fixtures/expected", filename))
			Expect(err).NotTo(HaveOccurred())
	...
	JustBeforeEach(func() {
		...
		filePaths = append(filePaths,
			filepath.Join(repo.RootDir(), "addons/packages/cert-manager/1.9.1/test/unittest/fixtures/values/default.yaml"),
		)
