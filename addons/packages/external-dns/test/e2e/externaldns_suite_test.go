...
var _ = BeforeSuite(func() {
	...
	packageNamespace = fmt.Sprintf("e2e-external-dns-package-%s", suffix)
	addonNamespace = fmt.Sprintf("e2e-external-dns-addon-%s", suffix)
	fixtureNamespace = fmt.Sprintf("e2e-external-dns-fixtures-%s", suffix)

	_, err := kubectl(nil, "create", "namespace", packageNamespace)
	Expect(err).NotTo(HaveOccurred())

	_, err = kubectl(nil, "create", "namespace", fixtureNamespace)
	Expect(err).NotTo(HaveOccurred())
})

var _ = AfterSuite(func() {
	_, err := kubectl(nil, "delete", "namespace", fixtureNamespace)
	Expect(err).NotTo(HaveOccurred())
