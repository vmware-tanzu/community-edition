...
func (o *CheckIfDockerDaemonAvailableURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on CheckIfDockerDaemonAvailableURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on CheckIfDockerDaemonAvailableURL")
	}
