...
const clusterTopologyStepFormSchema = yup.object({
    [FIELD_NAME_WORKLOAD_CLUSTER_NAME]: yup.string().nullable().required('Please enter a name for your workload cluster')
        .test('', 'Cluster name must contain only lower case letters and hyphen', value => value !== null && isK8sCompliantString(value)),
    [FIELD_NAME_WORKER_NODE_INSTANCE_TYPE]:
        yup.string().nullable().required('Please select an instance type for your workload cluster nodes')
}).required();
...
function InstanceTypeInList(instance: WorkerNodeInstanceType, register: any,
    ...
    return <>
        ...
        <RadioButton className="input-radio" cdsLayout="col:1" value={instance.id} register={register}
            name={FIELD_NAME_WORKER_NODE_INSTANCE_TYPE} onChange={onSelectNodeInstanceType} />
    ...
    return <div cds-layout="vertical gap:lg gap@md:lg col@sm:6 col:6" >
        <CdsFormGroup layout="vertical">
            <CdsInput layout="vertical">
                ...
                <input placeholder="workload-cluster-name" {...register(FIELD_NAME_WORKLOAD_CLUSTER_NAME)}
