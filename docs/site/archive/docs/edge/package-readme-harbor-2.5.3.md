...
This Harbor Package integrates open source Harbor 2.5.3. See [docs for Harbor 2.5.3](https://goharbor.io/docs/2.5.0/install-config/#harbor-components).
...
1. Configure Harbor Package
   ...
   Download the values.yaml file from [addons/packages/harbor/2.5.3/bundle/config/values.yaml](https://github.com/vmware-tanzu/community-edition/blob/main/addons/packages/harbor/2.5.3/bundle/config/values.yaml) to check all configuration values for Harbor Package and rename it to `harbor-values.yaml`.

   Or get the template configuration file by using script below:

   ```shell
   image_url=$(kubectl get packages harbor.community.tanzu.vmware.com.2.5.3 -o jsonpath='{.spec.template.spec.fetch[0].imgpkgBundle.image}')
   ...
   ```shell
   image_url=$(kubectl get package harbor.community.tanzu.vmware.com.2.5.3 -o jsonpath='{.spec.template.spec.fetch[0].imgpkgBundle.image}')
