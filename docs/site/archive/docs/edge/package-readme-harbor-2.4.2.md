...
1. Configure Harbor Package
   ...
   Users need to get the `harbor-values.yaml` file and specify the mandatory password/secrets in it to configure harbor.

   Firstly, download the values.yaml file from [addons/packages/harbor/2.4.2/bundle/config/values.yaml](https://github.com/vmware-tanzu/community-edition/blob/main/addons/packages/harbor/2.4.2/bundle/config/values.yaml) to check all configuration values for Harbor Package and rename it to `harbor-values.yaml`.
   Or get the template configuration file by using script below:

   ```shell
   image_url=$(kubectl get packages harbor.community.tanzu.vmware.com.2.4.2 -o jsonpath='{.spec.template.spec.fetch[0].imgpkgBundle.image}')
   ...
   Secondly, users need to specify the required password/secret field in the `harbor-values.yaml` file. We could either manually type in the values
   for password/secrets in `harbor-values.yaml` file. Or alternatively, generating values for these required field with the script below:
   ...
   Thirdly, please specify other Harbor configuration (e.g. admin password, hostname, persistence setting, etc.) in `harbor-values.yaml`.
...
The following lightwight pass-through values can be set to customize the Harbor package installation.
...
|` caBundleSecretName `| The custom ca bundle secret, the secret must contain key named "ca.crt", which will be injected into the trust store for core, jobservice, registry, trivy components. |  | string |
|` network.ipFamilies `| The array of network ipFamilies. The default value of [] actually means [IPv4,IPv6] | [] | array |
...
|` tlsCertificateSecretName `| The name of the secret for harbor FQDN if you want to use your own TLS certificate.  |             | string |
...
There is no additional application configuration required.

#### Multi-cloud configuration steps

There is no necessary steps required for multi-cloud configuration.

## What This Package Does

This package provides users with instructions/guidelines to install Harbor package in TCE cluster. Once Harbor package installed, users are able to push and pull OCI-compliant images to/from it.

## Components

This Harbor Package integrates open source Harbor 2.4.2. See [docs for Harbor 2.4.2](https://goharbor.io/docs/2.4.0/install-config/#harbor-components).
...
Harbor package requires a `harbor-data-values.yaml` file to configure it. Please navigate to [Package configuration values](#package-configuration-values) for more details of configuration settings.
An example of `harbor-data-values.yaml`:
...
secretKey: ASyV8BgiiSBdHdiH
database:
  password: aO4tGPsijsm4zbkk
  shmSizeLimit:
  maxIdleConns:
  maxOpenConns:
exporter:
...
core:
  replicas: 1
  secret: YrOutlydPXokUAKF
  xsrfKey: 1wOrdCNd91zaYANxLfX6g9N6qFqBCyyA
jobservice:
  replicas: 1
  secret: NIDg6lEODrvahRiC
registry:
  replicas: 1
  secret: I1Zp72oyoIupMIt1
notary:
  enabled: true
trivy:
  enabled: true
...
Harbor package doesn't support external Postgres and Redis.
