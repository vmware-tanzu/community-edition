...
## Prerequisites for Cluster Deployment {#prereqs}
...
## Create a Tanzu Kubernetes Cluster Configuration File {#config}
...
## Deploy a Tanzu Kubernetes Cluster with Minimum Configuration {#deploy}
...
1. Optionally set a name for the cluster in the `CLUSTER_NAME` variable.
   ...
   The `CLUSTER_NAME` value passed to `tanzu cluster create` overrides the name you set in the configuration file.
...
## Deploy a Cluster with Different Numbers of Control Plane and Worker Nodes {#deploy-counts}
...
## Configure Common Settings {#common-config}
...
## Deploy a Cluster in a Specific Namespace {#namespace}
...
## Create Tanzu Kubernetes Cluster Manifest Files {#manifest}
...
### Deploy a Cluster from a Saved Manifest File {#deploy-manifest}
...
## Advanced Configuration of Tanzu Kubernetes Clusters {#advanced}
